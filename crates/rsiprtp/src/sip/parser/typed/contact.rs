...
pub enum Contact {
    /// `Contact: *` — REGISTER wildcard (RFC 3261 §10.3).
    /// Only meaningful with `Expires: 0`; semantic check is
...
impl Contact {
    /// True if this is the wildcard form (`Contact: *`).
    ...
    /// `Contact: `).
    ///
    /// Detects the bare `*` wildcard form first. Otherwise
    /// delegates to [`NameAddr`] and wraps in
    /// [`Contact::Addr`].
    ...
    /// Lookup the `expires` parameter (RFC 3261 §20.10) — the
    /// preferred lifetime in seconds for this binding. Returns
    /// `None` for the wildcard form, for an absent param, or
    ...
    /// 1.0]`. Returns `None` for the wildcard form, for an
    /// absent param, or for a non-numeric value. We do NOT
    /// range-check; the registrar can.
