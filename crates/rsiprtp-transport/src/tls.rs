    ...
    async fn read_message(&mut self) -> Result<Option<Bytes>> {
        loop {
            ...
            if self.read_buf.len() > MAX_TLS_SIZE {
                return Err(rsiprtp_core::TransportError::MessageTooLarge {
...
fn load_private_key(path: &Path) -> Result<PrivateKeyDer<'static>> {
    ...
    Err(rsiprtp_core::TransportError::TlsError("No private key found in file".into()).into())
}
...
impl TlsTransport {
    ...
    pub async fn bind_server(addr: SocketAddr, config: TlsServerConfig) -> Result<Self> {
        ...
        let server_config = ServerConfig::builder()
            ...
            .map_err(|e| rsiprtp_core::TransportError::TlsError(e.to_string()))?;
    ...
    pub async fn connect(&self, addr: SocketAddr, server_name: &str) -> Result<()> {
        ...
        let connector = self.connector.as_ref().ok_or_else(|| {
            rsiprtp_core::TransportError::TlsError("No TLS connector configured".into())
        })?;
        ...
        let server_name = ServerName::try_from(server_name.to_string()).map_err(|_| {
            rsiprtp_core::TransportError::TlsError(format!("Invalid server name: {}", server_name))
        })?;
    ...
    pub async fn send(&self, msg: OutgoingMessage) -> Result<()> {
        ...
        .ok_or(rsiprtp_core::TransportError::ConnectionClosed)?;
